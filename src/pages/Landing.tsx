...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 56; i++) {
      ...
      b.style.cssText = `--dur:${(.4 + Math.random() * .7).toFixed(2)}s;--delay:${(Math.random() * .65).toFixed(2)}s`;
      wf.appendChild(b);
    }
  }, []);
  ...
  useEffect(() => {
    for (let d = 0; d < 7; d++) {
      ...
      for (let i = 0; i < 9; i++) {
        ...
        b.style.cssText = `--d:${(.5 + Math.random() * .8).toFixed(2)}s;--dl:${(i * .06 + Math.random() * .1).toFixed(2)}s;height:${16 + Math.random() * 28}px`;
        el.appendChild(b);
      }
    }
  }, []);
    ...
    <div className="lp-root">
      ...
      <nav className="lp-nav">
        ...
        <div className="lp-nav-right">
          <button onClick={goAuth} className="lp-btn-ghost-nav">Sign in</button>
          <button onClick={goAuth} className="lp-btn-nav">Start free →</button>
        </div>
      </nav>
              ...
              <div className="lp-score-body">
                <div className="lp-waveform-area">
                  ...
                  <div className="lp-transcript-box">
                    "So <span className="lp-fil">um</span> the main thing I wanted to highlight is that <span className="lp-hl">our Q3 results exceeded</span> expectations by <span className="lp-hl">23%</span>, and <span className="lp-fil">like</span> the team really <span className="lp-hl">delivered on every front</span>."
                  </div>
                  <div className="lp-tag-row">
                    <div className="lp-tag lp-tag-on">Strong opener</div>
                    <div className="lp-tag lp-tag-warn">2 filler words</div>
                    <div className="lp-tag lp-tag-on">3 power words</div>
                  </div>
                </div>
                <div className="lp-scores-area">
                  <div className="lp-overall-score">
                    <div className="lp-overall-num" id="lp-overall-counter">0</div>
      ...
      <section id="features" className="lp-section">
        <div className="lp-container">
          ...
          <div className="lp-sec-title">Everything you need to <span style={{ color: 'var(--lp-white)' }}>master your voice</span></div>
          <div className="lp-features-bento">
            <div className="lp-feat lp-feat-wide lp-reveal">
              ...
              <div className="lp-feat-visual">
                <div className="lp-chat-msg"><div className="lp-chat-avatar">AI</div><div className="lp-chat-bubble">Your pace was strong at 148 WPM — but I noticed 3 hedging phrases in 30 seconds. "I think", "kind of", "maybe". These undercut your credibility.</div></div>
                <div className="lp-chat-msg lp-chat-right"><div className="lp-chat-avatar">U</div><div className="lp-chat-bubble lp-chat-user">How do I stop saying "I think"?</div></div>
                <div className="lp-chat-msg"><div className="lp-chat-avatar">AI</div><div className="lp-chat-bubble">Replace with declaratives. Instead of "I think this is important" → "This is important." Try today's drill: 60 seconds on your work, zero hedges.</div></div>
              </div>
            </div>
      ...
      <footer className="lp-footer">
        <div className="lp-footer-logo">SYNT<span>E</span>RA</div>
...
.lp-logo {
  ...
  color: var(--lp-text); cursor: default;
}
...
.lp-btn-ghost-nav { background: none; border: none; color: var(--lp-muted); font-size: 0.85rem; cursor: pointer; transition: color 0.2s; font-family: 'DM Sans', sans-serif; }
.lp-btn-ghost-nav:hover { color: var(--lp-text); }
.lp-btn-nav {
  background: var(--lp-white); color: #050507;
...
.lp-paths-canvas { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
...
.lp-live-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--lp-accent); animation: lp-pulse 2s infinite; }
@keyframes lp-pulse { 0%,100%{opacity:1} 50%{opacity:0.3} }
.lp-hero h1 {
  font-family: 'Syne', sans-serif;
  font-size: clamp(3rem, 8vw, 6.5rem);
  font-weight: 800; line-height: 0.96; letter-spacing: -0.04em; max-width: 900px;
}
.lp-word { display: inline-block; margin-right: 0.16em; }
.lp-letter {
...
.lp-hero-sub { margin-top: 32px; color: var(--lp-muted); font-size: 1.05rem; max-width: 460px; line-height: 1.8; animation: lp-fadeUp 0.75s 0.6s ease both; }
.lp-hero-actions { margin-top: 48px; display: flex; gap: 12px; flex-wrap: wrap; justify-content: center; animation: lp-fadeUp 0.75s 0.7s ease both; }
.lp-btn-primary {
...
.lp-btn-secondary {
  display: inline-flex; align-items: center; gap: 8px;
  background: transparent; border: 1px solid var(--lp-border2);
  color: var(--lp-muted); font-family: 'DM Mono', monospace;
...
/* HERO VISUAL */
.lp-hero-visual { margin-top: 80px; width: 100%; max-width: 860px; animation: lp-fadeUp 0.8s 0.8s ease both; }
.lp-score-card {
  background: rgba(12,12,15,0.92); border: 1px solid var(--lp-border); border-radius: 14px; overflow: hidden;
  box-shadow: 0 48px 100px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.04);
...
.lp-wave-bar { flex: 1; border-radius: 1px; background: rgba(255,255,255,0.45); animation: lp-waveanim var(--dur,0.8s) ease-in-out infinite alternate; animation-delay: var(--delay,0s); }
@keyframes lp-waveanim { from{transform:scaleY(0.2)} to{transform:scaleY(1)} }
...
.lp-hl { color: rgba(255,255,255,0.7); }
.lp-fil { color: rgba(255,100,100,0.65); text-decoration: underline; text-decoration-style: wavy; }
...
.lp-chat-user { background:rgba(255,255,255,0.07); border-radius:8px 0 8px 8px; margin-left:auto; color:rgba(255,255,255,0.6); }
.lp-chat-right { flex-direction:row-reverse; }
...
.lp-sc-go { font-family:'DM Mono',monospace; font-size:0.6rem; color:rgba(255,255,255,0.35); }
...
.lp-footer-logo span { color:var(--lp-muted); }
